...
def batalla(ventana, jugador): 
    ...
    for nom, (r, c) in estilo.DATOS_BOTONES.items():
        nombre_boton = "Pokemons" if nom == "Cambiar" else nom
        botones[nombre_boton] = (r, c, estilo.fuente_nombre.render(nombre_boton, True, (255, 255, 255)).convert_alpha())

    nombre_rival = pokemon_rival.nombre
    texto_pantalla = "que pokemon elegiras"

    mensajes_acciones = {
        "Atacar": "Preparando el ataque...",
        "Objetos": "Abriendo la mochila de objetos...",
        "Pokemons": "A que Pokemon quieres llamar?" 
    }
    ...
    while ejecutando:
        ...
        for ev in pygame.event.get():
            if ev.type == pygame.QUIT:
                ...
                if ev.key == pygame.K_ESCAPE and MENU_ACTUAL != "PRINCIPAL" and not batalla_terminada and not TURNO_RIVAL_PENDIENTE:
                    MENU_ACTUAL = "PRINCIPAL"
                    if mi_pokemon is None:
                        texto_pantalla = "que pokemon elegiras"
                    else:
                        texto_pantalla = f"Que hara {mi_pokemon.nombre}?"
                ...
                if TURNO_RIVAL_PENDIENTE:
                    ...
                    texto_pantalla = f"{nombre_rival} enemigo uso {res_rival['nombre_ataque']}! Hizo {res_rival['daño']} de dano."
                    TURNO_RIVAL_PENDIENTE = False 
                    
                    if res_rival["debilitado"]:
                        pokemon_vivo = next((p for p in jugador.pokemons if p.vida_actual > 0), None)
                        if pokemon_vivo is None:
                            texto_pantalla = f"{mi_pokemon.nombre} se debilito! No te quedan Pokemon... Perdiste. (Clic para salir)"
                            aplicar_consecuencias_billetera(jugador, gano_jugador=False)
                            batalla_terminada = True
                        else:
                            texto_pantalla = f"{mi_pokemon.nombre} se debilito! Selecciona otro desde el boton Pokemons."
                            mi_pokemon = None
                            frames["mio"] = []
                            MENU_ACTUAL = "PRINCIPAL"
                    continue
                ...
                for nom, (rect, _, _) in btns.items():
                    if rect.collidepoint(mouse_pos):
                        ...
                        if MENU_ACTUAL == "PRINCIPAL":
                            if nom == "Atacar": 
                                if mi_pokemon is None:
                                    texto_pantalla = "No tienes ningun Pokemon en combate!"
                                elif mi_pokemon.vida_actual <= 0:
                                    texto_pantalla = f"{mi_pokemon.nombre} esta debilitado! Debes cambiar de Pokemon."
                                else:
                                    MENU_ACTUAL = "ATAQUES"
                            elif nom == "Objetos": 
                                if mi_pokemon is None:
                                    texto_pantalla = "Elige un Pokemon antes de usar objetos!"
                                else:
                                    MENU_ACTUAL = "OBJETOS"
                            elif nom == "Pokemons": 
                                MENU_ACTUAL = "POKEMONES"
                                texto_pantalla = mensajes_acciones["Pokemons"]
                                ...
                                if "Ataque" in nom:
                                    ...
                                    if res_jugador["debilitado"]:
                                        ...
                                        if idx_rival_activo < len(equipo_rival):
                                            ...
                                            idx["rival"] = 0
                                            texto_pantalla = f"{mi_pokemon.nombre} uso {res_jugador['nombre_ataque']}! El enemigo se debilito. El rival envia a {nombre_rival}! (Clic para continuar)"
                                            TURNO_RIVAL_PENDIENTE = True
                                        else:
                                            texto_pantalla = f"{mi_pokemon.nombre} uso {res_jugador['nombre_ataque']}! Derrotaste a todo el equipo rival! (Clic para salir)"
                                            aplicar_consecuencias_billetera(jugador, gano_jugador=True)
                                            batalla_terminada = True
                                    else:
                                        texto_pantalla = f"{mi_pokemon.nombre} uso {res_jugador['nombre_ataque']}! Hizo {res_jugador['daño']} de dano. (Clic para continuar)"
                                        TURNO_RIVAL_PENDIENTE = True
                                    ...
                                    if pokemon_seleccionado == mi_pokemon:
                                        ...
                                        era_inicio = mi_pokemon is None
                                        mi_pokemon = pokemon_seleccionado
                                        frames["mio"] = procesar_gif(mi_pokemon.gif_bytes, factor_escala=3.0)
                                        idx["mio"] = 0
                                        
                                        if era_inicio:
                                            texto_pantalla = f"Ve {mi_pokemon.nombre}! Que hara ahora?"
                                            MENU_ACTUAL = "PRINCIPAL"
                                            break
                                        else:
                                            texto_pantalla = f"Sali {mi_pokemon.nombre}! El rival aprovecha tu cambio para atacar. (Clic para continuar)"
                                            TURNO_RIVAL_PENDIENTE = True
        ...
        if mi_pokemon is not None:
            ...
            info_vida = f"HP: --/-- vs Rival ({idx_rival_activo + 1}/{len(equipo_rival)}): {pokemon_rival.vida_actual}/{pokemon_rival.stats['vida_max']}"
